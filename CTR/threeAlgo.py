...
df_data_train = pd.read_csv(r'D:\tencentvideos\交叉特征\train12.txt',header = None ,sep=',',encoding='gbk')
X_train = df_data_train.drop(len(df_data_train.keys())-1,axis = 1)
y_train = df_data_train[len(df_data_train.keys())-1]
df_data_test = pd.read_csv(r'D:\tencentvideos\交叉特征\test12.txt',header = None,sep=',',encoding='gbk')
...
# gbdt + lr result
lgb_train = lgb.Dataset(X_train_normalized,y_train)
lgb_eval = lgb.Dataset(X_test_normalized,y_test,reference=lgb_train)
...
lm = LogisticRegression(penalty='l2', C=0.01)
...
lg = LogisticRegression(penalty='l2',C=0.03)
