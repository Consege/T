...
class User(object):
    ...
    def getUserMat(self,user_list):
        user_mat = []
        user_actor_dict=dict()
        user_director_dict = dict()
        user_district_dict = dict()
        user_video_type_dict=dict()
        user_video_sub_type_dict = dict()
        user_age_dict = dict()
        # 获取挑选出来的兴趣\导演\演员等
        ...
        for line in user_list:
            ...
            if (len(user_profile) == 16):
                ...
                ############################
                user_actor_dict[user_profile[0]] = user_actor
                user_director_dict[user_profile[0]] = user_director
                user_district_dict[user_profile[0]] = user_district
                user_video_type_dict[user_profile[0]] = user_video_type
                user_video_sub_type_dict[user_profile[0]] = user_video_sub_type
                user_age_dict[user_profile[0]] = age  # 这里的age是用户出生年份
                ############################
            #print(len(user_mat[0]))
        # for x in user_mat:
        #     print(x)

        return  user_mat,user_actor_dict,user_director_dict,user_district_dict,user_video_type_dict,user_video_sub_type_dict,user_age_dict

    def getUsers(self,k):
        with open(r'D:\实验结果\推荐\大于30\大于30_users.txt',encoding='utf-8',errors='ignore') as user_profile:
            user_data = []
